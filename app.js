...
/***/ function(module, exports, __webpack_require__) {
	...
	_smooth_scrollbar.SmoothScrollbar.version = '5.6.5';
	...
	var __touchHandler = function __touchHandler() {
	    ...
	    var updateRecords = function updateRecords(evt) {
	        ...
	        _Object$keys(touchList).forEach(function (key) {
	            ...
	            var _getPosition = (0, _utils.getPosition)(touch);
	
	            var x = _getPosition.x;
	            var y = _getPosition.y;
	
	            var record = GLOBAL_TOUCHES[touch.identifier];
	
	            if (record) {
	                record.x = x;
	                record.y = y;
	            } else {
	                GLOBAL_TOUCHES[touch.identifier] = { x: x, y: y };
	            }
	        });
	    };
	    ...
	    this.__addEvent(container, 'touchmove', function (evt) {
	        ...
	        if (GLOBAL_TOUCHES[touchID].activeScrollbar && GLOBAL_TOUCHES[touchID].activeScrollbar !== _this) return;
	        ...
	        var duration = Date.now() - lastTouchTime || 1;
	
	        var last = lastTouchPos;
	        var current = lastTouchPos = (0, _utils.getPosition)(evt);
	
	        var disX = last.x - current.x;
	        var disY = last.y - current.y;
	
	        moveVelocity.x = disX / duration;
	        moveVelocity.y = disY / duration;
	
	        if (options.continuousScrolling && _this.__scrollOntoEdge(disX, disY)) {
	            return _this.__updateThrottle();
	        }
	
	        GLOBAL_TOUCHES[touchID].activeScrollbar = _this;
	
	        evt.preventDefault();
	
	        options.friction = 40; // change friction temporarily
	        _this.__addMovement(disX, disY);
	    });
